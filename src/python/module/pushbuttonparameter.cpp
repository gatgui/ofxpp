...
int PyOFXPushButtonParameterDescriptor_SetInteract(PyObject *self, PyObject *val, void*)
{
  ...
  {
    PyErr_SetString(PyExc_TypeError, "Tuple thrid element must be a sub class of ofx.Interact");
    return -1;
  }
  ...
  CATCH({desc->interact(PyOFX_GetInteractFunc(pluginClass, descClass, instClass));}, failed);
